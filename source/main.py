...
from EpipolarLines import EpipolarLine
...
def update_3d_plot(keypoint_list, ids):
    ax_3d.clear()
...
def update_2d_plot(keypoint_list, ids):
    ax_2d.clear()
    for keypoints_3d, obj_id in zip(keypoint_list, ids):
        color_rgb = normalize_rgb_color(id_to_rgb_color(obj_id))
        ax_2d.scatter(
...
tracker = Tracker(["yolo11x.pt", "yolo11x.pt"], [1, 2])
video_loader = VideoLoader(["camera_bananal/cam1.mp4", "camera_bananal/cam2.mp4"])
...
for i in range(video_loader.get_number_of_frames()):
    ...
    for match in matches:
        ...
        points_2d_centroids = [match_cam_0.single_centroid, match_cam_1.single_centroid]
    ...
    update_3d_plot(keypoints_3d_list, ids)
    update_2d_plot(keypoints_3d_list, ids)
